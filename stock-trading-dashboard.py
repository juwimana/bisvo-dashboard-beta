...
def main():
	...
	if help_btn:
		...
		# 		trade_columns = st.beta_columns([2.5,2.5,2.5,2.5])
		# 		buy = trade_columns[0].button('Buy')
		# 		sell = trade_columns[1].button('Sell')
		# 		refresh = trade_columns[3].button('Refresh')
		...
		# 			portfolio_df = portfolio_dataframe(open_df,ticker,price,shares,token,dividend,trade,path)
		# 		elif refresh:
		# 			portfolio_df = portfolio_dataframe(open_df,ticker,0,0,token,0,'refresh',path)
	...
	for symbol_string in symbol_strings:
		...
		if price > 0 and shares > 0 and trade == 'Buy':
			...
			if stocks.loc[ticker,"Total Cost"] > 0:
				stocks.loc[ticker,"Percent Return"] = stocks.loc[ticker,"Dollar Return"] / stocks.loc[ticker,"Total Cost"]
			else: 
		...
		stocks.loc[symbol,"Current Value"] = stocks.loc[symbol,"Total Number of Shares"] * stocks.loc[symbol,"Current Price"]
		stocks.loc[symbol,"Dollar Return"] = stocks.loc[symbol,"Current Value"] - stocks.loc[symbol,"Total Cost"] + stocks.loc[symbol,'Dividend']
		stocks.loc[symbol,"Percent Return"] = stocks.loc[symbol,"Dollar Return"] / stocks.loc[symbol,"Total Cost"]
